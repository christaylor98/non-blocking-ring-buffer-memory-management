...
fn run_sparse(n_readers: usize) -> (Stats, Stats, Stats)
{
    ...
    (go_cell(n_readers), go_mutex(n_readers), go_rwlock(n_readers))
}
...
fn run_heavy_write(n_readers: usize) -> (Stats, Stats, Stats) {
...
fn run_stress(n_readers: usize) -> (Stats, Stats, Stats) {
...
fn bench_sparse() {
    ...
    println!("╠═══════════╦══════════════╦══════════════╦══════════════╦══════════════╣");
    println!("║  readers  ║  Cell r/s    ║  Mutex r/s   ║  RwLock r/s  ║  Cell/Mutex  ║");
    println!("╠═══════════╬══════════════╬══════════════╬══════════════╬══════════════╣");

    for &n in &[3usize, 5, 10, 16] {
        let (c, m, r) = run_sparse(n);
        let speedup = (c.reads as f64 / c.elapsed) / (m.reads as f64 / m.elapsed);
        println!("║  {:>7}  ║  {:>10}  ║  {:>10}  ║  {:>10}  ║  {:>9.1}x  ║",
            n,
...
fn bench_heavy() {
    ...
    println!("╠═══════════╦══════════════╦══════════════╦══════════════╦══════════════╣");
    println!("║  readers  ║  Cell w/s    ║  Mutex w/s   ║  RwLock w/s  ║  Cell/Mutex  ║");
    println!("╠═══════════╬══════════════╬══════════════╬══════════════╬══════════════╣");

    for &n in &[3usize, 5, 10, 16] {
        let (c, m, r) = run_heavy_write(n);
        let speedup = (c.writes as f64 / c.elapsed) / (m.writes as f64 / m.elapsed);
        println!("║  {:>7}  ║  {:>10}  ║  {:>10}  ║  {:>10}  ║  {:>9.1}x  ║",
            n,
...
fn bench_stress() {
    ...
    println!("║  errors = epoch-inversion count per reader thread (must be 0 for Cell correctness)      ║");
    println!("╠═══════════╦══════════════╦════════╦══════════════╦══════════════╦══════════════╦════════╣");
    println!("║  readers  ║  Cell r/s    ║ errs   ║  Cell w/s    ║  Mutex r/s   ║  RwLock r/s  ║ C/M rx ║");
    println!("╠═══════════╬══════════════╬════════╬══════════════╬══════════════╬══════════════╬════════╣");

    for &n in &[3usize, 5, 10, 16] {
        let (c, m, r) = run_stress(n);
        let speedup = (c.reads as f64 / c.elapsed) / (m.reads as f64 / m.elapsed);
        println!("║  {:>7}  ║  {:>10}  ║ {:>6} ║  {:>10}  ║  {:>10}  ║  {:>10}  ║ {:>5.1}x ║",
            n,
    ...
    println!("╚═══════════╩══════════════╩════════╩══════════════╩══════════════╩══════════════╩════════╝");
    println!("  Note: Mutex/RwLock have no epoch, so their error column is N/A (always 0 by construction).");
}
...
fn main() {
    ...
    println!("  All multi-thread tests use Cell<u32> (inline path).");
    println!("  Cell<T> guarantees: single Release store / single Acquire load,");
    println!("  epoch travels with the value — no separate synchronisation atomic.");
